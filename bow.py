...
if __name__ == "__main__":
    ...
    plt.close()
    """
    #Oversampling classes that are underrepresented.
    pulse = train_dataset.loc[train_dataset['source'] == 'Market Pulse Navigator']
    town = train_dataset.loc[train_dataset['source'] == 'Town Hall']
    bio = train_dataset.loc[train_dataset['source'] == 'Bio Space']
    fin = train_dataset.loc[train_dataset['source'] == 'Financial Content']
    market = train_dataset.loc[train_dataset['source'] == 'Market Place']
    wsbn = train_dataset.loc[train_dataset['source'] == 'Wall Street Business Network']
    
    train_dataset = pd.concat([train_dataset, pulse, town, bio, fin, market, wsbn])

    """
    #Uncomment this to reproduce plot of dataset after oversampling
    dist = train_dataset['source'].value_counts()
    dist.plot(kind='bar')
    plt.show()
    plt.close()
